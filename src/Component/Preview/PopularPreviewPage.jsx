    ...
    <>
      <div className="w-full pb-15 px-4 pt-4 border-white p-4 bg-black">
        ...
        <div className="grid p-4 mx-auto border-white md:grid-cols-4 sm:grid-cols-2 gap-6 w-full max-w-7xl">
          { getRecommendation ? getRecommendation.map((item, index) => {
            return <div
              key={index}
            ...
            >
              <h1 className="text-black border-b absolute text-[14px] items-center top-0 px-2 font-medium w-fit bg-[#f7ff66] backdrop-blur-2xl rounded-full text-center  mt-2 flex">Recommendation<RiArrowRightDownLine className="size-5 " /></h1>
              ...
              <div className="bottom flex h-fit items-center border-white justify-between ">
                ...
                <span
                  onClick={() => {
                    navigate(`/popular/playing/${slugify(item.name || item.title)}`, {
                      state: item,
                    }), <a href='#'/>
                  }}
